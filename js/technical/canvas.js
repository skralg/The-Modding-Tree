	...
	for (id in data) {
		if (data[id].branches) {
			for (branch in data[id].branches) {
				drawTreeBranch(id, data[id].branches[branch], prefix + layer + "-", layer);
			}
		}
	}
}

function drawTreeBranch(num1, data, prefix, layer) { // taken from Antimatter Dimensions & adjusted slightly
	let num2 = data
	let color_id = 1
	let width = 15
	if (layer && tmp[layer].branchWidth) width = tmp[layer].branchWidth;
